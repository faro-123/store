...
app.post("/api/auth/register", async (c) => {
  ...
  try {
    const result = await c.env.DB.prepare("INSERT INTO users (username, password, email) VALUES (?, ?, ?)").bind(username, password, email || null).run();
...
app.post("/api/auth/login", async (c) => {
  ...
  const user = await c.env.DB.prepare("SELECT id, email FROM users WHERE username = ? AND password = ?").bind(username, password).first();
  if (user) {
...
app.post("/api/checkout", async (c) => {
  ...
  if (!userId || !productIds || productIds.length === 0) {
    return c.json({ success: false, message: "login and select products first" }, 400);
  }
  const resolvedIds = [];
  ...
  if (resolvedIds.length === 0) {
    return c.json({ success: false, message: "no valid products found" }, 400);
  }
...
app.post("/api/orders/simulate", async (c) => {
  ...
  if (!user) {
    ...
    await c.env.DB.prepare("INSERT INTO users (username, password) VALUES (?, 'pass123')").bind(username).run();
    const newUser = await c.env.DB.prepare("SELECT id FROM users WHERE username = ?").bind(username).first();
    const product2 = await c.env.DB.prepare("SELECT id, price FROM products ORDER BY RANDOM() LIMIT 1").first();
    if (product2) {
      await c.env.DB.prepare("INSERT INTO orders (user_id, product_id) VALUES (?, ?)").bind(newUser.id, product2.id).run();
    }
  ...
  await c.env.DB.prepare("INSERT INTO orders (user_id, product_id) VALUES (?, ?)").bind(user.id, product.id).run();
...
app.post("/api/products/:id/reviews", async (c) => {
  ...
  await c.env.DB.prepare("INSERT INTO reviews (product_id, rating, comment, username) VALUES (?, ?, ?, ?)").bind(productId, rating, comment || "", username).run();
  return c.json({ success: true });
});
app.get("/api/review-stats", async (c) => {
  const { results } = await c.env.DB.prepare(
    "SELECT r.product_id, ROUND(AVG(r.rating), 1) as avg_rating, COUNT(*) as count FROM reviews r GROUP BY r.product_id"
  ).all();
  return c.json(results);
});
...
// .wrangler/tmp/bundle-DzBgnP/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-DzBgnP/middleware-loader.entry.ts
